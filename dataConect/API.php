...
//DAMIAN HERNANDEZ MERINO
    ...
    /**
     * UpdateNotaAlumno(CalificacionId,Promedio)
     * Actualiza la tabla tbCalificaciones dato el CalificacionId y 
     * la nueva calificacion
     */
...
// redirect to success page
    ...
    function getCiclo() {
        ...
        include './conexion.php';
        $sql = "SELECT * FROM cliclo ORDER BY cliclo.idiciclo DESC";
...
// output data of each row
    ...
    function getPlanPago() {
        header('Content-Type: application/json');
        include './conexion.php';
        $sql = "SELECT * FROM plan_pago where estatus='Activo' ORDER BY idiplan DESC";
        $result = $conn->query($sql);
        $rows = array();
        if ($result->num_rows > 0) {
// output data of each row
...
// sql to delete a record
    ...
    function deletePlan() {
        ...
        if ($errorMSG == "") {
            include './conexion.php';
            $sql = "UPDATE plan_pago SET estatus = 'bloqueado' WHERE idiplan = $idiplan";
            if ($conn->query($sql) === TRUE) {
...
//echo "success";
    ...
    function getImageAlumnoByID($idialumno) {
        header('Content-Type: application/json');
        include './conexion.php';
        $sql = "SELECT idiarchivo, idialumno, nombre FROM archivo WHERE idialumno = $idialumno and titulo='perfil'";
        $result = $conn->query($sql);
        $rows = array();
        if ($result->num_rows > 0) {
// output data of each row
...
//UPDATE `archivo` SET `nombre` = '5c759d6f5e060.png' WHERE `archivo`.`idiarchivo` = 4
        if ($errorMSG == "") {
            ...
            $sql = "UPDATE archivo SET nombre = '" . $fileName . "' WHERE archivo.idialumno = $idialumno and titulo='perfil'";
            if ($conn->query($sql) === TRUE) {
...
// output data of each row
    ...
    function getNivel() {
        header('Content-Type: application/json');
        include './conexion.php';
        $sql = "SELECT * FROM cNiveles WHERE Estatus = 1";
        $result = $conn->query($sql);
        $rows = array();
        if ($result->num_rows > 0) {
// output data of each row
...
// output data of each row
    ...
    function getcardAlumnoAll() {
        ...
        $sql = "SELECT
                ...
                alumno.uso_factura,
                alumno.razon_social,
                alumno.doc_nacimiento_copia,
                alumno.doc_certificado_copia,
                alumno.doc_curp_copia,
                alumno.doc_ine_copia,
                alumno.doc_comprobante_copia,
                alumno.idicampus
                FROM
                alumno
...
//doc_nacimiento
        ...
        if ($errorMSG == "") {
            ...
            $sql = "SELECT
                    ...
                    INNER JOIN archivo ON archivo.idialumno = alumno.idialumno
                    INNER JOIN carrera ON alumno.idicarrera = carrera.idicarrera 
                    WHERE
                    archivo.titulo = 'perfil' 
                    AND alumno.idicarrera = $CarreraId";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
// output data of each row
                while ($row = $result->fetch_assoc()) {
                    ...
                    $data = array(
                        ...
                        'categoria' => $row["categoria"],
                        'contenido' => '<center><img width="160px;" src="dataConect/upload/' . $contenido . '"></center>',
                        'btnEditar' => '<a href="updateAlumno.php?p=' . $idigenerales . '&idialumno=' . $idialumno . '&idiarchivo=' . $idiarchivo . '"" target="_blank"><i class="pe-7s-note pe-2x pe-va" title="Editar"></i></a>',
                        'btnCredencial' => '<a href="credencialEstudiante.php?idialumno=' . $idialumno . '&idiarchivo=' . $idialumno . '" target="_blank"><i class="pe-7s-id pe-2x pe-va" title="Imprimir Credencial"></i></a></td>',
                    );
                    $rows['data'][] = $data;
                }
    ...
    function redirectToProfile() {
        ...
        if ($errorMSG == "") {
            ...
            $sql = "SELECT
                    ...
                    INNER JOIN datos_generales ON alumno.idigenerales = datos_generales.idigenerales
                    INNER JOIN archivo ON archivo.idialumno = alumno.idialumno
                    WHERE alumno.idialumno = $idialumno AND archivo.titulo = 'perfil'";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
                // output data of each row
...
// output data of each row
    ...
    function getDocumentosAlumno() {
        ...
        if ($errorMSG == "") {
            ...
            include './conexion.php';
            $sql = "SELECT
                    cDocumentosAlumnos.DocumentoAlumnoId,
                    cDocumentos.DocumentoId,
                    cDocumentos.Descripcion,
                    cDocumentosAlumnos.Entregado,
                    cDocumentosAlumnos.Nivel
                    FROM
                    cDocumentosAlumnos
                    INNER JOIN cDocumentos ON cDocumentosAlumnos.DocumentoSolicitadoId = cDocumentos.DocumentoId
                    where cDocumentosAlumnos.AlumnoId = $idialumno";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
                // output data of each row
...
// output data of each row
    ...
    function addAlumno() {
        ...
        if ($errorMSG == "") {
            ...
            include './conexion.php';
            $sql = "INSERT INTO alumno (idicampus, idicarrera, carrera, idigenerales, matricula, generacion, turno, codigo_credencial, cuatrimestre, idiuser, beca_colegiatura, vigencia) VALUES "
                    . "( '$campus' ,'$idicarrera ', '$carrera', '$idigenerales', '$matricula', '$periodo', '$turno',  '$codigo_credencial', '1', '$this->idiControlEscolar', '$beca_colegiatura', CURDATE());";
            if ($conn->query($sql) === TRUE) {
                $last_id = $conn->insert_id; // devuelve el id de registro
...
//beca_colegiatura
    ...
    function newDocument() {
        $errorMSG = "";
        //idialumno
        if (empty($_POST["AlumnoId"])) {
            $errorMSG = "AlumnoId is required ";
        } else {
        ...
        if ($errorMSG == "") {
            include './conexion.php';
            $sql = "INSERT INTO cDocumentosAlumnos(DocumentoSolicitadoId, AlumnoId, Entregado) VALUES ('$DocumentoId', '$AlumnoId', '1')";
            if ($conn->query($sql) === TRUE) {
                echo "success";
            } else {
                echo "Error: " . $sql . "<br>" . $conn->error;
...
// redirect to success page
    ...
    function getPeriodoByGrupoId() {
        ...
        if ($errorMSG == "") {
            header('Content-Type: application/json');
            include './conexion.php';
            $sql = "SELECT * FROM tbPeriodo where CicloId = $CicloId";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
                // output data of each row
...
//$sql = "SELECT g.GrupoId, g.Clave, g.Descripcion AS Grupo, cr.nombre AS Carrera, c.ciclo AS Ciclo, gr.Descripcion AS Grado, au.Descripcion AS Aula, t.Descripcion AS Turno, g.SituacionGrupoId AS Estatus, (SELECT COUNT(*) FROM tbAlumnoGrupo as alg where alg.GrupoId = g.GrupoId) as aforo FROM tbGrupos AS g, carrera AS cr, cliclo AS c, cGrados AS gr, cAulas AS au, cTurno AS t WHERE cr.idicarrera = g.CarreraId AND c.idiciclo = g.CicloId AND gr.GradosId = g.GradoId AND au.AulaId = g.AulaId AND t.TurnoId = g.TurnoId AND cr.idicarrera = $idicarrera $CicloId $TurnoId";
            $sql = "SELECT
                    ...
                    g.SituacionGrupoId AS Estatus,
                    ( SELECT COUNT( * ) FROM tbAlumnoGrupo AS alg WHERE alg.GrupoId = g.GrupoId ) AS aforo 
                    FROM
                    tbGrupos AS g,
                    carrera AS cr,
                    cliclo AS c,
                    ...
                    cTurno AS t 
                    WHERE
                    cr.idicarrera = g.CarreraId 
                    AND c.idiciclo = g.CicloId 
                    AND gr.GradosId = g.GradoId 
                    AND t.TurnoId = g.TurnoId 
                    AND cr.idicarrera = $idicarrera $CicloId $TurnoId";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
// output data of each row
    ...
    function getGruposCoursedByIdiAlumno() {
        ...
        if ($errorMSG == "") {
            ...
            $sql = "SELECT
                    ...
                    INNER JOIN cliclo ON tbGrupos.CicloId = cliclo.idiciclo
                    INNER JOIN cGrados ON tbGrupos.GradoId = cGrados.GradosId
                    WHERE
...
// output data of each row
    ...
    /**
     * SELECT * FROM tbMaterias WHERE GradoId
     */
...
// redirect to success page
        if ($errorMSG == "") {
            header('Content-Type: application/json');
            include './conexion.php';
            $sql = "SELECT * FROM tbMaterias WHERE GradoId = '$GradosId'";
            $result = $conn->query($sql);
            $rows = array();
            if ($result->num_rows > 0) {
// output data of each row
...
// redirect to success page
    ...
    function deleteDocument() {
        ...
        if ($errorMSG == "") {
            include './conexion.php';
            $sql = "DELETE FROM cDocumentosAlumnos WHERE DocumentoAlumnoId = $DocumentoAlumnoId";
            if ($conn->query($sql) === TRUE) {
                echo "success";
            } else {
                echo "Error: " . $sql . "<br>" . $conn->error;
...
// output data of each row
    ...
    /**
     * retorna una cosulta con la historia academica del estudianto,
     * solo muestra la calificacion final
     */
...
//$idinivel;
//$TurnoId;
                $this->creaMatrizHorario($MyGrupo, $idinivel, $TurnoId);
            } else {
                echo "Error updating record: " . $conn->error;
            }
            $conn->close();
    ...
    function getbAlumnoGrupoByIdGrupo() {
        ...
        if ($errorMSG == "") {
            ...
            $sql = "SELECT
                    ...
                    and archivo.titulo = 'perfil'
                    ORDER BY datos_generales.apellido_paterno ASC";
...
// redirect to success page
        if ($errorMSG == "") {
            ...
            include './conexion.php';
            $sql = "SELECT
                    gr.GradosId AS GradosId,
                    gr.Descripcion AS grado,
                    n.NivelId,
                    n.Descripcion AS nivel,
                    c.idicarrera as CarreraId,
                    c.nombre AS carrera,
                    c.idicarrera,
                    ci.idiciclo,
                    ci.ciclo AS ciclo,
