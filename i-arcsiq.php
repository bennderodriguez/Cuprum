...
<script type="text/javascript" src="asset/js/i-arcsiq.js"></script>
<div id="footer">
    <script>
...
//	xmlhttp.open("GET", "/MFG-RockJS/test.php&jwt=" + jwt.toLocaleString(), true);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xmlhttp.send("jwt=" + dbParam);
        });
    </script>
    ...
    <script>
        $("#Clientes").click(function () {
            console.log("Clientes Ajax");
            $("#Cobra").val("");
	//data: "action=getCliente&jwt=" + jwt.toLocaleString(),
                
            document.getElementById("loadTableClient").innerHTML = '<div class="alert alert-info"><strong>Espere</strong> Cargando Contenido ... espere <i class="pe-7s-config pe-spin pe-2x pe-va"></i></div>';
            $.ajax({
                type: "GET",
                url: host + "/Mfg-Rockjs/", //host variable global
                data: "action=getCliente&jwt=" + jwt.toLocaleString(),
                success: function (text) {
                    //var date = text.message;
	       	    var date = text;
                    //console.log(date);
                    ...
                    if (date.status == "Error") {
                        document.getElementById("help").innerHTML = '<div class="alert alert-warning"><strong>Mensaje!</strong> ' + date.message.auth + ' <button class="btn btn-outline-warning" onclick="louout()">Log in again</button></div>';
                        document.getElementById("loadTableClient").innerHTML = '<div class="alert alert-warning"><strong>Mensaje!</strong> ' + date.message.auth + ' <button class="btn btn-outline-warning" onclick="louout()">Log in again</button></div>';
                        $("#modalClientes .close").click()
                    }
                    txt += ' <table id="example" class="table table-striped table-bordered"  style="width:100%">';
                    txt += '<thead> <tr> <th>Cliente</th> <th>Nombre</th>  <th>Balance</th> <th>Abierto</th></tr> </thead>';
